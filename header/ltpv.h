...
unsigned int ltpv_buffer_pos[LTPV_NUMTHREADS];
...
{
    for (int i = 0; i < LTPV_NUMTHREADS; ++i)
    {
        ltpv_buffer_pos[i] = 0;
    }
}
...
{
    char s[200]="";
    sprintf(s, "[LTPV] %d %d %p %p", LTPV_NUMTHREADS_T, LTPV_SIZEBUFFER_T, (void*) ltpv_buffer, (void*) ltpv_buffer_pos);
    puts(s);
}
