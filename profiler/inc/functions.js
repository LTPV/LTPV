	...
	for(var idStream = 0; idStream < streams.length; idStream+=1) {
		if($("#titleStream-"+idStream).css("display")!='none') {
			...
			if(streams[idStream]!=undefined) for(var i = 0; i < streams[idStream].length; i++) {
				...
				started   = streams[idStream][i][2];
				ended     = streams[idStream][i][3];
				gws       = ''+streams[idStream][i][4];
				lws       = ''+streams[idStream][i][5];
				bw        = streams[idStream][i][6];
				queued    = streams[idStream][i][7];
				submitted = streams[idStream][i][8];
				size      = streams[idStream][i][9];
				//s = zoom*tWidth/(max-min);
				//begin = s*(started-min);
				//queuedG = s*(queued-min);
				//submittedG = s*(submitted-min);
				//end = s*(ended-min);
				...
				str+="><td class=\\\"tdLeft\\\">Queued at</td><td class=\\\"tdRight\\\">"+queued+unit+"</td></tr>";
				if(advanced==1) strNoHTML+="Queued at: "+queued+unit+"\n";
				str+="<tr class=\\\"advanced\\\"";
				if(advanced==0) str+="style=\\\"display:none;\\\""
				str+="><td class=\\\"tdLeft\\\">Submitted at</td><td class=\\\"tdRight\\\">"+submitted+unit+"</td></tr>";
				if(advanced==1) strNoHTML+="Submitted at: "+submitted+unit+"\n";
				str+="<tr><td class=\\\"tdLeft\\\">Started at</td><td class=\\\"tdRight\\\">"+started+unit+"</td></tr>";
				strNoHTML+="Started at: "+started+unit+"\n";
				str+="<tr><td class=\\\"tdLeft\\\">Ended at</td><td class=\\\"tdRight\\\">"+ended+unit+"</td></tr>";
				strNoHTML+="Ended at  : "+ended+unit+"\n";
				...
				if(gws!='')
					str+="<tr><td class=\\\"tdLeft\\\">Global_work_size</td><td class=\\\"tdRight\\\">["+gws+"]</td></tr>";
				if(lws!='')
					str+="<tr><td class=\\\"tdLeft\\\">Local_work_size</td><td class=\\\"tdRight\\\">["+lws+"]</td></tr>";
				if(size!='') {
				...
				<script>\
					...
					$("#taskInstance-'+idStream+'-'+i+'").click(function() {\
						cursor[0]='+started+';\
						fillInfos(\"'+str+'\", \"'+colors[task[1]][1]+'\");nameStarted=\"<span style=\\\"color:'+color+'\\\">'+task[0]+'.started</span>\";\
						startedVal='+started+';\
						...
						queuedSubmitted = ['+queued+','+submitted+'];\
					...
					$("#taskInstance-'+idStream+'-'+i+'").mouseover(function(e) {\
						...
						if(nameStarted!="") {\
							$("#console").html("<strong><span style=\\\"color:'+color+'\\\">'+task[0]+'.ended</span>&nbsp;-&nbsp;"+nameStarted+"&nbsp;=&nbsp;"+parseFloat('+ended+'-startedVal).toFixed(2)+unit+"</strong>");\
						}\
						showAdvanced();\
						refreshCanvasBackground();\
					});\
					...
					$("#taskInstance-'+idStream+'-'+i+'").dblclick(function(e) {\
						wLeft  = '+Math.max(min, parseFloat(started)-2*(parseFloat(ended)-parseFloat(started)))+';\
						wRight = '+Math.min(max, parseFloat(ended  )+2*(parseFloat(ended)-parseFloat(started)))+';\
						refreshScreen();\
					});\
				</script>';
			}
...
function refreshCanvasBackground() {
	...
	// Q and S (queued and submitted)
	if(queuedSubmitted[0]!=undefined && advanced==1) {
		var h=15;
		var w=15;
		var offsetY=35;
		for(k=0; k <2; k+=1) {
...
function createRAW() { // Fill the raw div with profiling informations
	...
	// Instanciated tasks
	output+='<table class="sortable" id="tabAdvanced"><thead><tr><th>task name</th><th>Instanciation name</th><th>stream</th><th class="sorttable_numeric" id="queuedAdvanced">queued</th><th class="sorttable_numeric">submitted</th><th class="sorttable_numeric">started</th><th class="sorttable_numeric">ended</th><th>global work size</th><th>local work size</th><th class="sorttable_numeric">bandwidth (MiB/s)</th><th class="sorttable_numeric">size (bytes)</th><th>size</th></tr></thead><tbody>';
	var tabTaskName = [];
	...
	$xml.find('device').each(function() {
		...
		$(this).find('stream').each(function() {
			...
			$(this).find('taskInstance').each(function() {
				...
				var queued=$(this).children('queued').text();
				var taskInstanceName = $(this).children('name').text();
				var submitted=$(this).children('submitted').text();
				var started=$(this).children('started').text();
				var ended=$(this).children('ended').text();
				...
				var gws=$(this).children('global_work_size').text();
				if(gws=='') gws='<span style="color:#303030">N/A</span>';
				else gws='['+gws+']';
				
				var lws=$(this).children('local_work_size').text();
				...
				output+='<td class="td2">'+queued+'</td>';
				output+='<td class="td2">'+submitted+'</td>';
				output+='<td class="td1">'+started+'</td>';
				output+='<td class="td2">'+ended+'</td>';
				...
				//output+='<td>'+$(this).children('queued').text()+'</td>';
				//][submitted='+$(this).children('submitted').text()+'][started='+$(this).children('started').text()+'][ended='+$(this).children('ended').text()+'][global_work_size='+$(this).children('global_work_size').text()+'][local_work_size='+$(this).children('local_work_size').text()+']';
				output+='</tr>';
			});
		});
	});
	...
	//var a= document.getElementById("queuedAdvanced"); sorttable.innerSortFunction.apply(a, []);
