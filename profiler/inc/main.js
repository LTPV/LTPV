...
var timelineWidth;
var timelineHeight;
//var queuedSubmitted=[undefined, undefined];
var queuedSubmitted=[undefined, undefined];
var $xml;
...
function init() {
	...
	if(filename=='') {
		window.location = loc+'?file=profiling.xml';
	...
	$xml.find('taskInstance').each(function() {
		a = parseFloat($(this).find('queued').text());
		b = parseFloat($(this).find('submitted').text());
		c = parseFloat($(this).find('started').text());
		d = parseFloat($(this).find('ended').text());

		if(a!=NaN) if(a > max || max==undefined) max=a;
		if(b!=NaN) if(b > max || max==undefined) max=b;
		if(c!=NaN) if(c > max || max==undefined) max=c;
		if(d!=NaN) if(d > max || max==undefined) max=d;

		if(a!=NaN) if(a < min || min==undefined) min=a;
		if(b!=NaN) if(b < min || min==undefined) min=b;
		if(c!=NaN) if(c < min || min==undefined) min=c;
		if(d!=NaN) if(d < min || min==undefined) min=d;
	});
		...
		$(this).find('stream').each(function() {
			...
			$(this).find('taskInstance').each(function() {
				...
				streams[idStreamG][idTaskInstance]=[
					...
					$(this).find('name').text(),
					$(this).find('started').text(),
					$(this).find('ended').text(),
					$(this).find('global_work_size').text(),
					$(this).find('local_work_size').text(),
					$(this).find('bandwidth').text(),
					$(this).find('queued').text(),
					$(this).find('submitted').text(),
					$(this).find('size').text(),
					];
				idTaskInstance += 1;
			}); 
